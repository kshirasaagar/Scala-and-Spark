...
object WikipediaRanking {
   ...
   */
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.map(_.text)
                                                                            .filter(_.split(" ").contains(lang))
                                                                            .collect()
                                                                            .length
  ...
  def rankLangs(langs: List[String],
                rdd: RDD[WikipediaArticle]): List[(String, Int)] = langs.map(lang => (lang,occurrencesOfLang(lang,rdd)))
                                                                        .sortWith(_._2 > _._2)
  ...
  def makeIndex(langs: List[String],
                ...
                RDD[(String, Iterable[WikipediaArticle])] = rdd.flatMap(x => langs.filter(lang => x.text.split(" ")
                                                                                                   .contains(lang))
                                                                                  .map(lang => (lang,x)))
                                                               .groupByKey()
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]):
                          List[(String, Int)] = index.map(lang => (lang._1,lang._2.size))
                                                     .sortBy(_._2)
                                                     .collect()
                                                     .toList
                                                     .reverse
  ...
  def rankLangsReduceByKey(langs: List[String],
                           rdd: RDD[WikipediaArticle]):
                           List[(String, Int)] = rdd.flatMap(x => langs.filter(lang => x.text.split(" ")
                                                                                        .contains(lang))
                                                                       .map(lang => (lang,1)))
                                                                       .reduceByKey(_+_)
